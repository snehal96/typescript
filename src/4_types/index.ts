...
export default (() => {
  ...
  function padLeft(value: string, padding: number | string): string {
    ...
    if( typeof padding == "string") return `${Array(padding + 1).join(' ')}${value}`
  ...
  function flatten(array:Array<number | Array<number>>) {
    const flattened = [];
  ...
  interface BirdLike extends Flyer {
  ...
  interface FishLike extends Swimmer {}
